...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            return citel.reply(img,{packname:'Mirage',author:'Quotely'},"sticker")
...
cmd({
        ...
        async(Void, citel, text) => {
            if (isNaN(text.split(" ")[0]) || !text) {
                let text = tiny(
                    "Fancy text generator\n\nExample: .fancy 32 Mirage\n\n"
                );
                listall("Mirage Bot").forEach((txt, num) => {
                    text += `${(num += 1)} ${txt}\n`;
                });
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply('Provide a link')
            try {
                link = text.split(" ")[0];
