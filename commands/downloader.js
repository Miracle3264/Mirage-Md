...
cmd({
            ...
            category: "downloader",
            filename: __filename,
            use: '<Hii,this is Mirage>',
        },
        async(Void, citel, text) => {
            if (!text) return citel.reply('Please give me a prompt.')
            let texttts = text
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply(`Use ${command} Faded - Alan Walker`);
            let yts = require("secktor-pack");
            let search = await yts(text);
            let anu = search.videos[0];
            let buttonMessage = {
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply(`Example: ${prefix}ringtone Faded - Alan Walker`)
