...
try {
    ...
    console.log("🌍 Connected to the Mirage DB")
} catch {
...
let cc = Config.sessionName.replace(/Mirage;;;/g, "");
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === "connecting") {
                   console.log("ℹ️ ᴹᶦʳᵃᵍᵉ Connecting to WhatsApp... Please Wait.");
                }
                if (connection === 'open') {
...
<html>
  ...
  <body>
    <section>
      Hello from Mirage Tech!!
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`Mirage Server listening on port http://localhost:${port}!`));
