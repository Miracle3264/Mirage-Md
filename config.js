const fs = require('fs-extra')
if (fs.existsSync('config.env')) require('dotenv').config({ path: __dirname+'/config.env' });

//═══════[Required Variables]════════\\
global.owner = process.env.OWNER_NUMBER ||  2347013159244,
global.mongodb = process.env.MONGODB_URI || "mongodb+srv://miracle32669:Iyanu1234@mirage.0zogh.mongodb.net/?retryWrites=true&w=majority&appName=Mirage"
global.port= process.env.PORT || 5000
global.email = 'miracle32669@outlook.com'
global.github = 'https://github.com/M3264/Mirage-Md'
global.location = 'Nigeria'
global.gurl = 'https://instagram.com/' // add your username
global.sudo = process.env.SUDO || '2347013159244'
global.devs = '2347013159244';
global.website = 'https://github.com/M3264/Mirage-Md' //wa.me/+23400000000000
global.THUMB_IMAGE = process.env.THUMB_IMAGE || 'https://telegra.ph/file/f5945740d07991c7ae698.jpg'
module.exports = {
  botname:   process.env.BOT_NAME || 'ᴹᶦʳᵃᵍᵉ' : process.env.BOT_NAME,
  ownername: process.env.OWNER_NAME || 'M3264' : process.env.OWNER_NAME,
  sessionName:  process.env.SESSION_ID || Mirage;;;' : process.env.SESSION_ID,
  author:  process.env.PACK_INFO.split || 'M3264' : process.env.PACK_INFO ,
  auto_read_status :  process.env.AUTO_READ_STATUS || true : process.env.AUTO_READ_STATUS,
  packname:  process.env.PACK_INFO.split || 'Mirage-Md' : process.env.PACK_INFO,
  autoreaction:  process.env.AUTO_REACTION  || false : process.env.AUTO_REACTION,
  antibadword :  process.env.ANTI_BAD_WORD ||  'true' : process.env.ANTI_BAD_WORD,
  alwaysonline:  process.env.ALWAYS_ONLINE || 'true' : process.env.ALWAYS_ONLINE,
  antifake : process.env.FAKE_COUNTRY_CODE || '971' : process.env.FAKE_COUNTRY_CODE,
  readmessage:  process.env.READ_MESSAGE || false : process.env.READ_MESSAGE,
  caption : process.env.CAPTION || 'Generated with 🖤' : process.env.CAPTION,
  auto_status_saver: process.env.AUTO_STATUS_SAVER || 'true' : process.env.AUTO_STATUS_SAVER,
  HANDLERS:  process.env.PREFIX || '.' : process.env.PREFIX,
  warncount : process.env.WARN_COUNT || 3 : process.env.WARN_COUNT,
  disablepm:  process.env.DISABLE_PM || false : process.env.DISABLE_PM,
  levelupmessage:  process.env.LEVEL_UP_MESSAGE || false : process.env.LEVEL_UP_MESSAGE,
  antilink:  process.env.ANTILINK_VALUES || 'chat.whatsapp.com' : process.env.ANTILINK_VALUES,
  antilinkaction: process.env.ANTILINK_ACTION || 'remove' : process.env.ANTILINK_ACTION,
  BRANCH: 'main', 
  ALIVE_MESSAGE:  process.env.ALIVE_MESSAGE || '`Hallo👋 Mirage-Md is Alive $ Active`' : process.env.ALIVE_MESSAGE,
  autobio:  process.env.AUTO_BIO || false : process.env.AUTO_BIO,
  OPENAI_API_KEY:  process.env.OPENAI_API_KEY || false : process.env.OPENAI_API_KEY,
  heroku:  process.env.heroku || false : process.env.heroku,
  HEROKU: {
    HEROKU: process.env.HEROKU || false,
    API_KEY: process.env.HEROKU_API_KEY || '' : process.env.HEROKU_API_KEY,
    APP_NAME: process.env.HEROKU_APP_NAME || '' : process.env.HEROKU_APP_NAME
},
  VERSION: process.env.VERSION || 'v.0.0.1' : process.env.VERSION,
  LANG: process.env.THEME || 'MIRAGE',
  WORKTYPE: process.env.WORKTYPE || 'public' : process.env.WORKTYPE
};


let file = require.resolve(__filename)
fs.watchFile(file, () => {
	fs.unwatchFile(file)
	console.log(`Update'${__filename}'`)
    delete require.cache[file]
	require(file)
})
